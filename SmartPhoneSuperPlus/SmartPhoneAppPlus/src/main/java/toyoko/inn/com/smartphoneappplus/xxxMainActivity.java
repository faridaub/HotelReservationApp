...
import toyoko.inn.com.smartphoneappplus.xxxProduct.SubCategory;
import toyoko.inn.com.smartphoneappplus.xxxProduct.SubCategory.ItemList;
 ...
 *
 */
public class xxxMainActivity extends Activity {
	
	private ArrayList<xxxProduct> pProductArrayList;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.xxxactivity_main);
		...
		pProductArrayList=new ArrayList<xxxProduct>();
		pProductArrayList.add(new xxxProduct("Emotions", pSubItemArrayList));
		pProductArrayList.add(new xxxProduct("Garments", pSubItemArrayList2));
		...
		for (int i = 0; i < pProductArrayList.size(); i++) {
			...
			View mLinearView = inflater.inflate(R.layout.xxxrow_first, null);
			...
			mImageArrowFirst.setBackgroundResource(R.drawable.ic_arrow_next);
			}
			else{
				mLinearScrollSecond.setVisibility(View.VISIBLE);
				mImageArrowFirst.setBackgroundResource(R.drawable.ic_arrow_down);
			}
		    ...
		    for (int j = 0; j < pProductArrayList.get(i).getmSubCategoryList().size(); j++) {
				...
				View mLinearView2 = inflater2.inflate(R.layout.xxxrow_second, null);
				...
				if(isSecondViewClick==false){
					...
					mImageArrowSecond.setBackgroundResource(R.drawable.ic_arrow_next);
					}
					else{
						mLinearScrollThird.setVisibility(View.VISIBLE);
						mImageArrowSecond.setBackgroundResource(R.drawable.ic_arrow_down);
					}
				 ...
				 */
				  for (int k = 0; k < pProductArrayList.get(i).getmSubCategoryList().get(j).getmItemListArray().size(); k++) {
						...
						View mLinearView3 = inflater3.inflate(R.layout.xxxrow_third, null);
